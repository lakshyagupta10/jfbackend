...
class Today_OrderController extends Controller
{
   ...
   public function today_order_parcel(Request $request)
    {
    	 ...
    	 $vendor_id = $vendor->vendor_id;			
    	  ...
    	  $todayorder  =   DB::table('orders')
    	                   ...
    	                   // ->join('vendor_area', 'area.area_id','=', 'vendor_area.area_id')
    	                    ...
    	                    ->select('orders.order_id','orders.order_id','orders.user_id','orders.delivery_date','tbl_user.user_name','orders.dboy_id','orders.delivery_charge', 'orders.total_price','orders.total_products_mrp','orders.delivery_charge','delivery_boy.delivery_boy_name','orders.dboy_id','orders.order_status','orders.cart_id','orders.delivery_date','user_address.user_number','user_address.user_name','user_address.address','orders.time_slot','orders.delivery_charge','orders.paid_by_wallet','orders.rem_price','orders.price_without_delivery','orders.coupon_discount')
							->where('orders.vendor_id', $vendor_id)
							->whereDate('orders.order_date', $currentDate)
    	                    ->groupBy('orders.order_id','orders.order_id','orders.user_id','orders.delivery_date','tbl_user.user_name','orders.dboy_id','orders.delivery_charge', 'orders.total_price','orders.total_products_mrp','orders.delivery_charge','delivery_boy.delivery_boy_name','orders.dboy_id','orders.order_status','orders.cart_id','orders.delivery_date','user_address.user_number','user_address.user_name','user_address.address','orders.time_slot','orders.delivery_charge','orders.paid_by_wallet','orders.rem_price','orders.price_without_delivery','orders.coupon_discount')
    	                    ->orderBy('user_id')
    	                    ->get(); 
//    	dd($todayorder);
    	 ...
    	 $vendordelivery_boy =  DB::table('delivery_boy')
    	               //   ->join('delivery_boy_area', 'delivery_boy.delivery_boy_id', '=', 'delivery_boy_area.delivery_boy_id')
						  ->select('delivery_boy.delivery_boy_id' , 'delivery_boy.delivery_boy_name' )
    	                    ->GroupBy('delivery_boy.delivery_boy_id' , 'delivery_boy.delivery_boy_name')
        ...
        return view('parcel.oder_incentive.today_order', compact("vendor_email","details", "vendor","todayorder","delivery_boy"));
	 }
	else
	 {
    ...
    public function parcel_next_day(Request $request)
    {
      ...
      if(Session::has('vendor'))
      {
    	 ...
    	 $vendor_id = $vendor->vendor_id;			
    	  $nextdayorder  =   DB::table('orders')
    	                    ...
    	                    ->where('orders.vendor_id', $vendor_id)
    	                    ->groupBy('area.area_id','orders.order_id','orders.order_id','orders.user_id','orders.delivery_date','tbl_user.user_name','orders.dboy_id','orders.delivery_charge', 'orders.total_price','orders.total_products_mrp','orders.delivery_charge','delivery_boy.delivery_boy_name','orders.dboy_id','orders.order_status','orders.cart_id','orders.delivery_date','user_address.user_number','user_address.user_name','user_address.address','orders.time_slot','orders.delivery_charge','orders.paid_by_wallet','orders.rem_price','orders.price_without_delivery','orders.coupon_discount')
    	                    ->orderBy('user_id')
    	                    ->get(); 
        ...
        if(count($delivery_boy)>0){
         ...
         return view('parcel.oder_incentive.next_day_order', compact("vendor_email","details", "vendor","nextdayorder","delivery_boy","boy_area_id"));
	 }
	else
	 {
	    return redirect()->route('parcellogin')->withErrors('please login first');
...
public function parcel_complete_order(Request $request)
    ...
    //	dd($nextdayorder);
        ...
        if(count($delivery_boy)>0){
         ...
         return view('parcel.oder_incentive.complete_order', compact("vendor_email","details", "vendor","nextdayorder","delivery_boy","boy_area_id"));
