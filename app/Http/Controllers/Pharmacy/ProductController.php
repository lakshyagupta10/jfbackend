    ...
    if(Session::has('vendor'))
     {
        ...
        $currency =  DB::table('currency')
               ->select('currency_sign')
                ->paginate(10);         
        return view('pharmacy.product.product',compact("vendor_email","product","vendor","currency"));
	 }
	else
	 {
     ...
     public function Addproduct(Request $request)
    {
      if(Session::has('vendor'))
      {
        ...
        $subcat= DB::table('resturant_category')
         ...
         return view('pharmacy.product.addproduct',compact("vendor_email","subcat","vendor"));
	 }
	else
	 {
	    return redirect()->route('vendorlogin')->withErrors('please login first');
      ...
      {	
       $product_id=$request->product_id;
    	 ...
    	 $subcat=DB::table('resturant_category')
                 ->where('vendor_id', $vendor->vendor_id)
                ->get();
    	 return view('pharmacy.product.Editproduct',compact("vendor_email","vendor","product","product_id","subcat"));
	 }
	else
	 {
	    return redirect()->route('vendorlogin')->withErrors('please login first');
