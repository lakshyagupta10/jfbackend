    ...
    if(Session::has('vendor'))
     {
        ...
        $currency =  DB::table('currency')
               ->select('currency_sign')
                ->paginate(10);
        return view('resturant.product.product',compact("vendor_email","product","vendor","currency"));
	 }
	else
	 {
     ...
     public function Addproduct(Request $request)
    {
      if(Session::has('vendor'))
      {
        ...
        $subcat= DB::table('resturant_category')
         ...
         return view('resturant.product.addproduct',compact("vendor_email","subcat","vendor"));
	 }
	else
	 {
	    return redirect()->route('vendorlogin')->withErrors('please login first');
      ...
      if(Session::has('vendor'))
      {
       $product_id=$request->product_id;
    	 ...
    	 $subcat=DB::table('resturant_category')
                 ->where('vendor_id', $vendor->vendor_id)
                ->get();
    	 return view('resturant.product.Editproduct',compact("vendor_email","vendor","product","product_id","subcat"));
	 }
	else
	 {
	    return redirect()->route('vendorlogin')->withErrors('please login first');
    	...
    	if(Session::has('vendor'))
          {
               ...
               if($productname!=null){
                  ...
                  return view('resturant.product.product',compact("vendor_email","product","vendor"));

               }else{

                $product= DB::table('resturant_product')
                         ->join('resturant_category','resturant_product.subcat_id', '=', 'resturant_category.resturant_cat_id')
                         ->where('resturant_product.vendor_id', $id)
                        ->get();
