...
class UserController extends Controller
{
    ...
    public function deleteaccount(Request $request)
    {
		...
		{
		    $message = array('status'=>'1', 'message'=>'Something went wrong');
                return $message;
		}
    }    
    ...
    public function upload_id(Request $request)
    {
        ...
        if($request->id_proof){
            ...
            $id_proof = str_replace('data:image/png;base64,', '', $id_proof);
            $fileName = str_replace(" ", "-", $id_proof);
            $fileName = date('dmyHis').'user_image'.'.'.'png';
            $fileName = str_replace(" ", "-", $fileName);
            \File::put(public_path(). '/images/user/proof/' . $fileName, base64_decode($id_proof));
            $id_proof = 'images/user/proof/'.$fileName;
        }
            else{
                $id_proof = 'N/A';
            }
        $updated =  DB::table('tbl_user')
                    ->where('user_id', $user_id)
                    ->update(['id_proof'=>$id_proof]);
                    if($updated)
                    {
                            $message = array('status'=>1, 'message'=>"Id proof updated ");
                            return $message;
                    }
                    else
                    {
     ...
     public function check_id(Request $request)
    {
        ...
        $updated =  DB::table('tbl_user')
                    ->where('user_id', $user_id)
                    ->first();
                    if($updated->id_proof!='N/A')
                    {
                            $message = array('status'=>1, 'message'=>"Id proof exists ");
                            return $message;
                    }
                    else
                    {
